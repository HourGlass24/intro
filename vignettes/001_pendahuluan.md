...
``` r
dim(iris) # Mengecek dimensi dari data
```

    ## [1] 150   5

``` r
str(iris) # Mengecek struktur datri data
```
...
``` r
colnames(iris) # Mengecek name colom
```
...
``` r
head(iris) # Mengecek data teratas
```
...
``` r
head(iris, 10) # Mengecek 10 data teratas
```
...
``` r
tail(iris) # Mengecek data terakhir
```
...
``` r
tail(iris, 10) # Mengecek 10 data terakhir
```
...
``` r
summary(iris) # Mengecek rangkuman data
```
...
``` r
iris[c(1, 6, 12),] # Mencetak Barus 1 6 12
```
...
``` r
iris[2,2] # subset atau ekstrak data pada kolom 2 baris 2
```
...
``` r
class(iris$Species) # Mencetak class data
```

    ## [1] "factor"

``` r
length(iris$Petal.Width) # Banyak data pada kolim tertentu
```

    ## [1] 150

``` r
levels(iris$Species) # Mencetak atribut
```
...
``` r
unique(iris$Species) # Vector berupa number
```
...
``` r
mean(iris$Sepal.Length) # Mencari Mean
```

    ## [1] 5.843333

``` r
sd(iris$Sepal.Length) # Mencari Standar Deviasi
```

    ## [1] 0.8280661

``` r
median(iris$Sepal.Width) # Mencari Median
```

    ## [1] 3

``` r
sum(iris$Petal.Length) # Mencari Sum
```

    ## [1] 563.7

``` r
plot(iris) # Menampilkan Plot
```

![](001_pendahuluan_files/figure-gfm/unnamed-chunk-5-1.png)<!-- -->

``` r
cor(iris[, -5]) # Mencari korelasi
```
...
``` r
write.csv(iris, file = "iris1.csv", row.names = FALSE) # menyimpan file
```
